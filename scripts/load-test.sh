...
#AWS_DIRECTORIES="ebs-local-storage-run efs-single-zone-run efs-regional-run same-az-lustre-run cross-az-lustre-run same-az-zfs-run rhel8-nfs-same-subnet-run same-az-ontap-run multi-az-zfs-run"
AWS_DIRECTORIES="same-az-zfs-run"
